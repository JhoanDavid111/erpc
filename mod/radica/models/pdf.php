...
//Metodos CRUD
	...
	public function personOne($perid){
		$sql ="SELECT p.perid, p.nodocemp, p.pernom, p.perape, p.peremail, p.depid, a.valnom AS are, p.ordgas, p.actemp, p.planta, p.cargo, c.valnom AS carg FROM persona AS p LEFT JOIN valor AS c ON p.cargo=c.valid LEFT JOIN valor AS a ON p.depid=a.valid WHERE p.perid='".$perid."' ORDER BY p.pernom, p.perape";
	...
	public function getFirma($norad,$visres){
		$sql ="SELECT DISTINCT norad, fecres, perid FROM radresp WHERE norad='".$norad."' AND visres IN (".$visres.") ORDER BY fecres";
	...
	public function getFirmap($norad,$visres){
		$sql ="SELECT DISTINCT norad, perid FROM radresp WHERE norad='".$norad."' AND visres IN (".$visres.") ORDER BY fecres";
		//echo "<br><br><br>".$sql."<br>'".$norad."'<br><br><br>"; 
		$execute = $this->db->query($sql);
		$save = $execute->fetchall(PDO::FETCH_ASSOC);
