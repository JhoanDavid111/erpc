...
//Metodos CRUD
	...
	public function getTotal($fecin, $fecfi, $trad=0){
	...
	public function updMem(){		
		$sql = "UPDATE configuracion SET nomemo={$this->consecutivo} WHERE noradf='1';";	
	...
	public function updOfi(){		
		$sql = "UPDATE configuracion SET noofi={$this->consecutivo} WHERE noradf='1';";	
		$update= $this->db->prepare($sql);
		$arrdata = NULL;
		$save=$update->execute();
	...
	public function updExt(){		
		$sql = "UPDATE configuracion SET noext={$this->consecutivo} WHERE noradf='1';";	
		$update= $this->db->prepare($sql);
		$arrdata = NULL;
		$save=$update->execute();
	...
	public function getAllTipo($tiprad){
		$sql = "SELECT r.norad, r.asurad, r.carrad, c.valnom as cargo, r.noradext, r.orirad, r.firrad, r.folrad, r.tiprad, v.valnom as tip, r.areprorad, a.valnom as are, r.noradcon, r.regrad, p.pernom, p.perape, r.fecrad, r.emprad, r.nomrad, r.dirrad, r.posrad, r.ubiid, u.ubinom, u.ubidepto, r.cuerad, r.revrad, r.coprad, r.chkrad, r.adjrad, r.consecutivo, r.mfirrad, r.oficot FROM radicado AS r INNER JOIN persona AS p ON r.regrad=p.perid INNER JOIN valor AS v ON r.tiprad=v.valid LEFT JOIN valor AS c ON r.carrad=c.valid LEFT JOIN valor AS a ON r.areprorad=a.valid LEFT JOIN ubica AS u ON r.ubiid=u.ubiid WHERE r.tiprad=$tiprad ORDER BY r.fecrad DESC";
	...
	public function getOne(){
		...
		//echo "<br><br><br>".$sql."<br>'".$ano."','".$this->norad."'<br><br><br>"; 
		$execute = $this->db->query($sql);
...
//Es el mismo setVal
	...
	public function save(){
		$sql= "INSERT INTO radicado (asurad, carrad, noradext, orirad, firrad, folrad, tiprad, areprorad, noradcon, regrad, fecrad, emprad, nomrad, dirrad, posrad, ubiid, cuerad, revrad, coprad, chkrad, adjrad, consecutivo, carradofi, mfirrad, oficot) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		$insert = $this->db->prepare($sql);
		$arrdata = array($this->getAsurad(), $this->getCarrad(), $this->getNoradext(), $this->getOrirad(), $this->getFirrad(), $this->getFolrad(), $this->getTiprad(), $this->getAreprorad(), $this->getNoradcon(), $this->getRegrad(), $this->getFecrad(), $this->getEmprad(), $this->getNomrad(), $this->getDirrad(), $this->getPosrad(), $this->getUbiid(), $this->getCuerad(), $this->getRevrad(), $this->getCoprad(), $this->getChkrad(), $this->getAdjrad(), $this->getConsecutivo(), $this->getCarradofi(), $this->getMfirrad(),$this->getOficot());
		// echo $sql;
		// var_dump($arrdata);
		// die();
	...
	public function edit(){		
		...
		$sql = "UPDATE radicado SET asurad=?, carrad=?, noradext=?, orirad=?, firrad=?, folrad=?, tiprad=?, areprorad=?, noradcon=?, regrad=?, fecrad=?, emprad=?, nomrad=?, dirrad=?, posrad=?, ubiid=?, cuerad=?, revrad=?, coprad=?, chkrad=?, adjrad=?, consecutivo=?, carradofi=?, oficot=?";
		$sql .= " WHERE norad={$this->norad};";	

		$update= $this->db->prepare($sql);
		$arrdata = array($this->getAsurad(), $this->getCarrad(), $this->getNoradext(), $this->getOrirad(), $this->getFirrad(), $this->getFolrad(), $this->getTiprad(), $this->getAreprorad(), $this->getNoradcon(), $this->getRegrad(), $this->getFecrad(), $this->getEmprad(), $this->getNomrad(), $this->getDirrad(), $this->getPosrad(), $this->getUbiid(), $this->getCuerad(), $this->getRevrad(), $this->getCoprad(), $this->getChkrad(), $this->getAdjrad(), $this->getConsecutivo(), $this->getCarradofi(), $this->getOficot());
	...
	public function getConfig(){
		$sql ="SELECT noradf, nit, nomemp, dircon, mosdir, telcon, mostel, celcon, moscel, emacon, mosema, logocon, nomemo, noofi, noext FROM configuracion";
		//echo "<br><br><br>".$sql."<br><br><br>"; 
		$execute = $this->db->query($sql);
		$save = $execute->fetchall(PDO::FETCH_ASSOC);
	...
	public function editConfO($noofi){		
		$sql = "UPDATE configuracion SET noofi='$noofi' WHERE noradf=1;";	
		$update= $this->db->prepare($sql);
		$save=$update->execute();
	...
	public function editConfE($noext){		
		$sql = "UPDATE configuracion SET noext='$noext' WHERE noradf=1;";	
		$update= $this->db->prepare($sql);
		$save=$update->execute();
	...
	public function editConfM($nomemo){		
		$sql = "UPDATE configuracion SET nomemo='$nomemo' WHERE noradf=1;";	
		$update= $this->db->prepare($sql);
		$save=$update->execute();
...
/*
	...
	public function seltrd2(){
		...
		$sql="SELECT deptrd, doctrd, destrd, tiptrd, ordtrd, agentrd, acentrd, dfintrd FROM trd WHERE doctrd<500 ORDER BY destrd" ;
		//echo "<br><br><br>".$sql."<br><br>";
	...
	public function seltrd2Sub($serie){
	...
	public function seltrd3Sub($serie){
	...
	public function seltrd4Sub($serie){
...
*/
	...
	public function resp($norad){
		$sql ="SELECT r.idres, r.norad, r.fecres, r.perid, u.pernom, u.perape, u.peremail, r.idpro, p.nompro, r.idflu, f.actflu, r.leido, r.obsres, r.estrad, v.valnom AS est, r.visres, w.valnom AS vist FROM radresp AS r INNER JOIN persona AS u ON r.perid=u.perid INNER JOIN proceso AS p ON r.idpro=p.idpro INNER JOIN flujo AS f ON r.idflu=f.idflu INNER JOIN  valor AS v ON r.estrad=v.valid INNER JOIN  valor AS w ON r.visres=w.valid WHERE r.norad='$norad'";
		//echo "<br><br><br>".$sql."<br>'".$norad."'<br><br><br>"; 
		$execute = $this->db->query($sql);
		$save = $execute->fetchall(PDO::FETCH_ASSOC);
	...
	public function personAll(){
		$sql ="SELECT p.perid, p.nodocemp, p.pernom, p.perape, p.peremail, p.depid, a.valnom AS are, p.ordgas, p.actemp, p.planta, p.cargo, c.valnom AS carg FROM persona AS p INNER JOIN valor AS a ON p.depid=a.valid LEFT JOIN valor AS c ON p.cargo=c.valid WHERE p.actemp=1 AND p.perid<>265 ORDER BY p.pernom, p.perape";
		//echo "<br><br><br>".$sql."<br><br><br>"; 
		$execute = $this->db->query($sql);
		$save = $execute->fetchall(PDO::FETCH_ASSOC);
	...
	public function personOne($perid){
		...
		//echo "<br><br><br>".$sql."<br>'".$perid."'<br><br><br>"; 
		$execute = $this->db->query($sql);
		$save = $execute->fetchall(PDO::FETCH_ASSOC);
...
// Realizado 22/06/2022 Inicio ---------------
	...
	public function savrr(){
		$sql= "INSERT INTO radresp(norad, fecres, perid, idpro, idflu, leido, obsres,estrad, visres) VALUES (?,?,?,?,?,?,?,?,?)";
		$insert = $this->db->prepare($sql);
		$arrdata = array($this->getNorad(), $this->getFecres(), $this->getPerid(), $this->getIdpro(), $this->getIdflu(), $this->getLeido(), $this->getObsres(), $this->getEstrad(), $this->getVisres());
...
// Realizado 22/06/2022 Fin ---------------
	public function exilei($perid, $idres){
		$sql="SELECT idres FROM radlei WHERE perid='$perid' and idres='$idres';";
		//echo "<br><br><br>".$sql."<br>'".$idres."','".$leido."'<br><br>";
		$execute = $this->db->query($sql);
		$save = $execute->fetchall(PDO::FETCH_ASSOC);
		// var_dump($save);
		// $error= $this->db->errorInfo();
	...
	public function updlei($perid, $idres, $visto){
		$sql="UPDATE radlei SET visto='$visto' WHERE perid='$perid' and idres='$idres';";
		//echo "<br><br><br>".$sql."<br>'".$idres."','".$leido."'<br><br>";
		$execute = $this->db->query($sql);
		$save = $execute->fetchall(PDO::FETCH_ASSOC);
		// var_dump($save);
	...
	public function seltra($norad){
		$sql="SELECT MAX(idres) AS id FROM radresp WHERE norad='$norad';";
		//echo "<br><br><br>".$sql."<br>'".$norad."'<br><br>";
		$execute = $this->db->query($sql);
		$save = $execute->fetchall(PDO::FETCH_ASSOC);
	...
	public function seltraper($norad,$perid){
		$sql="SELECT MAX(idres) AS id FROM radresp WHERE norad='$norad' AND perid='$perid';";
		//echo "<br><br><br>".$sql."<br>'".$norad."'<br><br>";
		$execute = $this->db->query($sql);
		$save = $execute->fetchall(PDO::FETCH_ASSOC);
	...
	public function updtrz1($norad, $leido){
		$sql="UPDATE radresp SET leido=:leido WHERE norad='$norad';";
		//echo "<br><br><br>".$sql."<br>'".$norad."','".$leido."'<br><br>";
		$execute = $this->db->query($sql);
		$save = $execute->fetchall(PDO::FETCH_ASSOC);
	...
	public function updtrz2($idres, $leido){
		$sql="UPDATE radresp SET leido=$leido WHERE idres='$idres';";
		// echo "<br><br><br>".$sql."<br>'".$idres."','".$leido."'<br><br>";
		$execute = $this->db->query($sql);
		$save = $execute->fetchall(PDO::FETCH_ASSOC);
	...
	public function inslei($perid, $idres){
		$sql="INSERT INTO radlei VALUE ('$perid', '$idres', '1');";
		//echo "<br><br><br>".$sql."<br>'".$perid."','".$idres."'<br><br>";
		$execute = $this->db->query($sql);
		$save = $execute->fetchall(PDO::FETCH_ASSOC);
	...
	public function seleres($perid,$norad){
		//$sql="SELECT idres FROM radresp WHERE perid='$perid' AND norad='$norad'";
		$sql="SELECT idres FROM radresp WHERE norad='$norad'";
		//echo "<br><br><br>".$sql."<br>'".$perid."','".$norad."'<br><br>";
		$execute = $this->db->query($sql);
		$save = $execute->fetchall(PDO::FETCH_ASSOC);
	...
	public function seleido($perid,$idres){
		$sql="SELECT perid, idres, visto FROM radlei WHERE perid='$perid' AND idres='$idres'";
		//echo "<br><br><br>".$sql."<br>'".$perid."','".$idres."'<br><br>";
		$execute = $this->db->query($sql);
		$save = $execute->fetchall(PDO::FETCH_ASSOC);
	...
	public function straza($norad, $fecres, $estrad, $perid){
		$sql="SELECT idres FROM radresp WHERE norad='$norad' AND fecres='$fecres' AND estrad='$estrad' AND perid='$perid'";
		//echo "<br><br><br>".$sql."<br>'".$norad."','".$fecres."','".$estrad."','".$perid."'<br><br>";
		$execute = $this->db->query($sql);
		$save = $execute->fetchall(PDO::FETCH_ASSOC);
