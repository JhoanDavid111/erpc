...
<?php $url_acttraz = base_url."radica/savetra"; ?>
	...
	<form class="m-tb-40" action="<?=base_url;?>radica/expedientes&save='yes'" method="POST" enctype="multipart/form-data">
		<div class="row">
			<div class="form-group col-md-4">
				<label for="anoexp">Año</label>
				<input type="number" class="form-control" id="anoexp" name="anoexp" value="2022" placeholder="2022" >
			</div>
			<div class="form-group col-md-4">
				<label for="doctrd">Subserie</label>
				<select id="doctrd" name="doctrd" class="form-control form-control-sm" style="padding: 0px;" required>							
					<?php foreach($dttipd as $dttpd): ?>
						<?php 
							$long = strlen($dttpd["doctrd"]);
							$esp = "";
							for($i=0;$i<$long;$i++){
								$esp .= "&nbsp;";
							}
						 ?>
						<option value="<?=$dttpd["doctrd"]?>"><?=$esp.$dttpd["destrd"];?></option>
					<?php endforeach; ?>					
				</select>
			</div>
			<div class="form-group col-md-4">
				<label for="actexp">Activo</label>
				<select id="actexp" name="actexp" class="form-control form-control-sm" style="padding: 0px;" >							
