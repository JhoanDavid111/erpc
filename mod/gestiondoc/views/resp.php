					...
					if($personPlan){
						foreach ($personPlan as $do){ ?>
			                <option value="<?=$do['perid'];?>" 
			                	<?=isset($val) && $do['perid']==$val['nomrad'] ? ' selected ' : ''; ?>><?=$do['pernom']." ".$do['perape']." - ".$do['carg'];?>
			                </option>
		...
		<form class="m-tb-40" action="<?=$url_action?>" method="POST" enctype="multipart/form-data">
			<div class="row">
				...
				<div class="form-group col-md-6" id="go2">
					...
					<textarea name="obsres" id="obsres" class="form-control" required></textarea>
				</div>
				<div class="form-group col-md-6" id="go2">
