...
//Es el mismo setVal
	...
	public function seltrd3Sub($serie,$depid,$j){
		...
		$sql="SELECT deptrd, doctrd, destrd, tiptrd, ordtrd, agentrd, acentrd, dfintrd FROM trd WHERE area LIKE '%$depid%' AND deptrd LIKE '".$serie."%'  AND LENGTH(doctrd)>=7 AND LENGTH(doctrd)<=8 ";
	...
	public function seltrdExpCompJuri($depid){
		...
		$sql="SELECT * FROM docgestion INNER JOIN trd ON docgestion.ultserie=trd.doctrd WHERE compjuridica=$depid AND carpeta=1 ";	
	...
	public function agregadoc($perid,$depid,$nomar,$tipoarchivo,$peso,$fecha,$rutafinal,$ultserie,$num,$nomserie,$dfin,$depende,$fcierre,$anio,$fsis){

		
		$sql = "INSERT INTO docgestion (depid,perid,nomar,tipo,peso,fecha,ruta,ultserie,num,nomserie,dfin,depende,fcierre,fsis) VALUES ($depid,$perid,'$nomar','$tipoarchivo',$peso,'$fecha','$rutafinal',$ultserie,$num,'$nomserie','$dfin','$depende','$fcierre','$fsis')";
	...
	public function getar2($anio,$area){
				
		$sql="SELECT DISTINCT ultserie,depid,fecha, valor.valnom, trd.destrd, trd.acentrd, trd.dfintrd, trd.agentrd FROM docgestion INNER JOIN valor ON docgestion.depid=valor.valid INNER JOIN trd ON docgestion.ultserie=trd.doctrd  WHERE YEAR(fcierre) = '$anio' AND carpeta = 2 GROUP BY ultserie ";		
...
";		
	...
	public function nomasSerie($serie){
		$sql = "SELECT * FROM `trd` WHERE `doctrd` like '%$serie%' and CHARACTER_LENGTH(doctrd)>5";	
...
// Realizado 22/06/2022 Inicio ---------------
	...
	public function getAreasJ(){
		$sql ="SELECT * FROM valor WHERE valid>1000 AND parid=1 ORDER BY valnom ASC" ;		
		// var_dump($sql);
		// $error= $this->db->errorInfo();
		// die();
		$execute = $this->db->query($sql);
