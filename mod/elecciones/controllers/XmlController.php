...
class xmlController{
	...
	public function full(){		
		...
		$html = NULL;
		$btn = isset($_POST["btn"]) ? $_POST["btn"]:"AL"; //"GO";
		$dpt = isset($_POST["dpt"]) ? $_POST["dpt"]:16; //15;
		$mnc = isset($_POST["mnc"]) ? $_POST["mnc"]:1; //0;
		$crp = isset($_POST["crp"]) ? $_POST["crp"]:3; //1;
		$ctd = isset($_POST["ctd"]) ? $_POST["ctd"]:30; //1;
		$nmnc = isset($_POST["nmnc"]) ? $_POST["nmnc"]:NULL;
		$nb = isset($_POST["nb"]) ? $_POST["nb"]:0;
		$cfg = isset($_POST["cfg"]) ? $_POST["cfg"]:NULL;

		$mbas = new Mbas();
		if($btn && $dpt && $crp){
			$mbas->updCfg($btn,$dpt,$mnc,$crp,$ctd,$nmnc,$nb,$cfg);
		}
	...
	public function digital(){		
		Utils::useraccess('xml/digital',$_SESSION['pefid']);
		$html = NULL;
		$btn = isset($_POST["btn"]) ? $_POST["btn"]:"AL"; //"GO";
		$dpt = isset($_POST["dpt"]) ? $_POST["dpt"]:16; //15;
		$mnc = isset($_POST["mnc"]) ? $_POST["mnc"]:1; //0;
		...
		if (file_exists(path_file.$arc)) {
		    ...
		    if($btn=="AL"){ //ALCALDÍA
				$html .= '<div class="form-group col-md-3">';
					$html .= '<a href="../views/vcsv.php?arc='.substr($arc,5).'&nu=4" target="_blank"><button class="btn-primary-ccapital">ALCALDÍA</button></a>';
				    $html .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
				$html .= '</div>';
				...
				$html .= '<div class="form-group col-md-3">';
					$html .= '<a href="../views/vcsv.php?arc='.substr($arc,5).'&nu=4" target="_blank"><button class="btn-primary-ccapital">CONCEJO MUNICIPAL</button></a>';
				    $html .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
				$html .= '</div>';
				// $html .= '<div class="form-group col-md-3">';
				...
				$html .= '<div class="form-group col-md-3">';
					$html .= '<a href="../views/vcsv.php?arc='.substr($arc,5).'&nu=4" target="_blank"><button class="btn-primary-ccapital">ASAMBLEA DEPARTAMENTAL</button></a>';
				    $html .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
				$html .= '</div>';
				// $html .= '<div class="form-group col-md-3">';
				...
				$html .= '<div class="form-group col-md-3">';
					$html .= '<a href="../views/vcsv.php?arc='.substr($arc,5).'&nu=4" target="_blank"><button class="btn-primary-ccapital">CAMARA</button></a>';
				    $html .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
				$html .= '</div>';
				// $html .= '<div class="form-group col-md-3">';
				...
				$html .= '<div class="form-group col-md-3">';
				    $html .= '<a href="../views/vcsv.php?arc='.substr($arc,5).'&nu=1" target="_blank"><button class="btn-primary-ccapital">SENADO</button></a>';
				    $html .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
				$html .= '</div>';
				// $html .= '<div class="form-group col-md-3">';
				...
				$html .= '<div class="form-group col-md-3">';
				    $html .= '<a href="../views/vcsv.php?arc='.substr($arc,5).'&nu=2" target="_blank"><button class="btn-primary-ccapital">CITREP</button></a>';
				    $html .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
				$html .= '</div>';
				// $html .= '<div class="form-group col-md-3">';
...
// ---------------------------------------------------
							$podd = strpos($dtdd,intval($Tlb->Departamento['V']));
							$podm = strpos($dtdm,intval($Tlb->Municipio['V']));
							if(intval($Tlb->Departameto['V']) and intval($Tlb->Municipio['V'])==0){ 
								$podd = strpos($dtdm,intval($Tlb->Departamento['V']));
...
// ---------------------------------------------------
							...
							if($vbnom){
										...
										$mbas->setElecdep(intval($Tlb->Departamento['V']));
										$mbas->setElecmun(intval($Tlb->Municipio['V']));
										// if($tip<>'00'){
										// 	$mbas->insResulm();
										// }else{
										// 	$mbas->delResul();
										// 	$mbas->insResul();
										// }
										if((($podd>-1 && $podm>-1) || ($podd>-1 && intval($Tlb->Municipio['V'])==0))){
...
// ---------------------------------------------------
							$html .= '<tr>';
								...
								$dtpa = $mbas->getPar($TBol->Partido['V']);
								$html .= '<td>'.$TBol->Partido['V'].' ';
									if($dtpa AND $dtpa[0]["elenp"]) $html .= $dtpa[0]["elenp"]; else $html .= '';
								$html .= '</td>';
								$html .= '<td>'.$TBol->Sec['V'].'</td>';
								$html .= '<td>'.$xml->Boletin->Departamento['V'].' - '.$xml->Boletin->Desc_Departamento ['V'].'</td>';
								$html .= '<td>'.$xml->Boletin->Municipio['V'].' - '.$xml->Boletin->Desc_Municipio['V'].'</td>';
...
////////////////////////////////////////////////////////////////////////
							...
							if($TBol->Candidato['V'] && (($podd>-1 && $podm>-1) || ($podd>-1 && intval($Tlb->Municipio['V'])==0))){


// echo intval($Tlb->Departamento['V'])." ".intval($Tlb->Municipio['V'])."<br>";
// echo $Tlb->Desc_Departamento['V']." ".$Tlb->Desc_Municipio['V']."<br>";
// echo $Tlb->Desc_Municipio['V']." ".$podd." ".$podm."<br>";
// echo 'if('.$TBol->Candidato['V'].' && (('.$podd.'>-1 && '.$podm.'>-1) || ('.$podd.'>-1 && '.intval($Tlb->Municipio['V']).'==0))) LE ENTRO<br>';
							    if($btn=="AL") $mbas->insResulm("eleral"); //ALCALDÍA
								elseif($btn=="GO") $mbas->insResulm("elergo"); //GOBERNACIÓN
								elseif($btn=="CO") $mbas->insResulm("elerco"); //CONCEJO MUNICIPAL
								elseif($btn=="AS") $mbas->insResulm("eleras"); //ASAMBLEA DEPARTAMENTAL
