...
//Metodos CRUD
 ...
 //   				. " WHERE dt.idpaa = ".$this->idpaa;
	...
	public function getAllrespuesta($tipo){
		// $sql = "SELECT s.*, v.*, r*, v.valnom as tipoe FROM derautpre as s INNER JOIN valor as v ON s.valid=v.valid INNER JOIN derautres r ON s.iddap = r.iddap WHERE s.tipo='$tipo' AND s.leido=".$this->leido;
	...
	public function getOne(){
		$sql ="SELECT s.*, v.*, p.*, v.valnom as tipoe FROM derautpre as s INNER JOIN valor as v ON s.valid=v.valid INNER JOIN persona as p ON s.perid=p.perid WHERE s.tipo='da' AND iddap=".$this->iddap;
		$execute = $this->db->query($sql);
		//echo $sql;
